...
Purpose:
یہ فائل اے آئی اسسٹنٹ کے لیے محفوظ، project-based اور developer-style
local memory فراہم کرتی ہے۔

اہم خصوصیات:
- Unlimited projects (صرف available storage تک)
- ہر project کی الگ memory
- Project files کا record
- Tasks اور progress
- Tests اور test results
- Failed tests اور failures
- Solutions اور development decisions
...
- Item text retrieving for Copying
- Automatic backups
...
- Clear All AI Memory
- Retest awareness
- Crash-safe SQLite storage

IMPORTANT:
یہ module خود سے عام project files delete نہیں کرتا۔
Destructive memory operations کے لیے explicit user confirmation لازم ہے۔
"""
...
def utc_now():
    """موجودہ UTC وقت محفوظ format میں واپس کرتا ہے۔"""
...
def _storage_root():
    """
    Local memory کے لیے private application storage location۔
    اگر AI_ASSISTANT_PRIVATE_STORAGE موجود ہو تو اسے استعمال کیا جائے گا۔
    ورنہ user home کے اندر .ai_assistant/local_developer_memory استعمال ہوگا۔
    """
...
def connect():
    """محفوظ SQLite connection بناتا ہے۔"""
...
def initialize_database():
    """Local memory database تیار کرتا ہے۔"""
    ...
    try:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS tests (
                ...
                last_tested_at TEXT,
                code_state_hash TEXT NOT NULL DEFAULT '',
                notes TEXT NOT NULL DEFAULT '',
                created_at TEXT NOT NULL,
                updated_at TEXT NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS failures (
                ...
                title TEXT NOT NULL,
                description TEXT NOT NULL DEFAULT '',
                attempted_solution TEXT NOT NULL DEFAULT '',
                result TEXT NOT NULL DEFAULT '',
        ...
        if active is None:
            ...
            if first is None:
                ...
                cur = conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    ("Default Project", "Default local project.", "ACTIVE", 0, now, now)
                )
    ...
    try:
        with transaction() as conn:
            cur = conn.execute(
                ...
                VALUES (?, ?, 'ACTIVE', 0, ?, ?)
                """,
                (name, str(description), now, now)
            )
            project_id = cur.lastrowid
            conn.execute(
                "INSERT OR REPLACE INTO metadata (key, value) VALUES ('active_project_id', ?)",
                (str(project_id),)
            )
...
def upsert_file(project_id, path, purpose="", status="ACTIVE", content_hash="", notes=""):
    ...
    with transaction() as conn:
        conn.execute(
            """
            INSERT INTO files (project_id, path, purpose, status, content_hash, last_changed_at, notes)
            VALUES (?, ?, ?, ?, ?, ?, ?)
...
def add_task(project_id, title, description="", priority="NORMAL"):
    ...
    with transaction() as conn:
        cur = conn.execute(
            ...
            VALUES (?, ?, ?, 'PENDING', ?, ?, ?)
            """,
            (project_id, title, str(description), str(priority), now, now)
        )
...
def add_test(project_id, test_name, purpose="", code_state_hash=""):
    ...
    with transaction() as conn:
        conn.execute(
            """
            INSERT INTO tests (project_id, name, purpose, status, code_state_hash, created_at, updated_at)
            VALUES (?, ?, ?, 'PENDING', ?, ?, ?)
            ON CONFLICT(project_id, name) DO UPDATE SET
                purpose=excluded.purpose,
                code_state_hash=excluded.code_state_hash,
                updated_at=excluded.updated_at
            """,
            (project_id, name, str(purpose), str(code_state_hash), now, now)
        )
        row = conn.execute(
            "SELECT id FROM tests WHERE project_id=? AND name=?",
            (project_id, name)
        ).fetchone()
        return row["id"]
...
def record_test_result(test_id, result, status="PASSED", failure_reason="", code_state_hash="", notes=""):
    ...
    with transaction() as conn:
        conn.execute(
            """
            UPDATE tests
            SET result=?, status=?, failure_reason=?, last_tested_at=?, code_state_hash=?, notes=?, updated_at=?
            WHERE id=?
            """,
            (str(result), str(status).upper(), str(failure_reason), now, str(code_state_hash), str(notes), now, test_id)
        )
    return True
...
def add_failure(project_id, title, description="", attempted_solution="", result=""):
    ...
    with transaction() as conn:
        cur = conn.execute(
            """
            INSERT INTO failures (project_id, title, description, attempted_solution, result, status, created_at)
            VALUES (?, ?, ?, ?, ?, 'OPEN', ?)
            """,
            (project_id, str(title), str(description), str(attempted_solution), str(result), now)
        )
    return cur.lastrowid
...
def resolve_failure(failure_id, result=""):
    with transaction() as conn:
        conn.execute(
            "UPDATE failures SET status='RESOLVED', result=?, resolved_at=? WHERE id=?",
            (str(result), utc_now(), failure_id)
        )
    return True
...
def add_decision(project_id, title, decision, reason="", status="APPROVED"):
    ...
    with transaction() as conn:
        cur = conn.execute(
            """
            INSERT INTO decisions (project_id, title, decision, reason, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (project_id, str(title), str(decision), str(reason), str(status), now, now)
        )
    return cur.lastrowid
...
def record_change(project_id, summary, file_path="", reason="", result=""):
    with transaction() as conn:
        cur = conn.execute(
            """
            INSERT INTO changes (project_id, file_path, summary, reason, result, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (project_id, str(file_path), str(summary), str(reason), str(result), utc_now())
        )
    return cur.lastrowid
...
def save_session(project_id, summary="", work_done="", work_remaining="", stopped_at="", next_action=""):
    ...
    with transaction() as conn:
        cur = conn.execute(
            """
            INSERT INTO sessions (project_id, summary, work_done, work_remaining, stopped_at, next_action, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (project_id, str(summary), str(work_done), str(work_remaining), str(stopped_at), str(next_action), now)
        )
        conn.execute(
            """
            UPDATE projects
            SET last_worked_at=?, paused_at=?, next_action=?, updated_at=?
            WHERE id=?
            """,
            (now, (str(stopped_at) if stopped_at else None), str(next_action), now, project_id)
        )
    return cur.lastrowid
...
def get_latest_session(project_id):
    conn = connect()
    try:
        row = conn.execute("SELECT * FROM sessions WHERE project_id=? ORDER BY id DESC LIMIT 1", (project_id,)).fetchone()
        return _dict(row)
    finally:
        conn.close()
...
def add_log(project_id, message, category="GENERAL", level="INFO"):
    with transaction() as conn:
        cur = conn.execute(
            """
            INSERT INTO logs (project_id, category, message, level, created_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (project_id, str(category), str(message), str(level), utc_now())
        )
    return cur.lastrowid


# ============================================================
# MEMORY ITEMS & COPY HELPERS
# ============================================================
...
def add_memory_item(category, title, content, project_id=None):
    ...
    with transaction() as conn:
        cur = conn.execute(
            """
            INSERT INTO memory_items (project_id, category, title, content, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (project_id, str(category), str(title), str(content), now, now)
        )
    return cur.lastrowid
...
def get_memory_item_content(item_id, item_type="chat"):
    """
    کاپی کرنے کے لیے مخصوص ائٹم یا میسج کا متن (Content) حاصل کرتا ہے۔
    """
    conn = connect()
    try:
...
def add_chat_message(project_id, role, content):
    with transaction() as conn:
        cur = conn.execute(
            """
            INSERT INTO chats (project_id, role, content, created_at)
            VALUES (?, ?, ?, ?)
            """,
            (project_id, str(role), str(content), utc_now())
        )
    return cur.lastrowid
...
def list_project_chats(project_id, limit=100):
    ...
    try:
        rows = conn.execute(
            "SELECT * FROM chats WHERE project_id=? ORDER BY id DESC LIMIT ?",
            (project_id, limit)
        ).fetchall()
        rows.reverse()
        return [_dict(row) for row in rows]
...
# ============================================================
# USER-CONTROLLED SINGLE & BULK DELETE
# ============================================================
...
def delete_bulk_memory_items(item_ids_list, item_type="chats", user_confirmation=False):
    """
    UI پر ٹک مارک (Checkboxes) کے ذریعے منتخب کردہ متعدد ائٹمز کو ایک ساتھ ڈیلیٹ کرتا ہے۔
    """
    _require_user_confirmation(user_confirmation)
...
def clear_project_memory(project_id, user_confirmation=False):
    """
    AI/developer memory صاف کرتا ہے۔
    IMPORTANT: Project record اور file records محفوظ رہتے ہیں۔
    """
    _require_user_confirmation(user_confirmation)
    with transaction() as conn:
        tables = ("memory_items", "tests", "failures", "decisions", "changes", "sessions", "logs", "chats", "tasks")
        for table in tables:
...
def clear_all_ai_memory(user_confirmation=False):
    """
    تمام AI/developer memory صاف کرتا ہے۔
    IMPORTANT: Projects اور project file records محفوظ رہتے ہیں۔
    """
    _require_user_confirmation(user_confirmation)
    with transaction() as conn:
        tables = ("memory_items", "tests", "failures", "decisions", "changes", "sessions", "logs", "chats", "tasks")
...
def get_project_context(project_id):
    """AI کو developer-style project context فراہم کرتا ہے۔"""
    project = get_project(project_id=project_id)
    if not project:
        return None
...
def cleanup_old_backups(max_backups=MAX_BACKUPS):
    try:
        ...
        backups = [
            p for p in BACKUP_FOLDER.iterdir()
            if p.is_file() and p.name.startswith("project_memory_backup_") and p.suffix == ".db"
        ]
...
def export_memory_json(output_path):
    ...
    try:
        ...
        for project in projects:
            pid = project["id"]
            item = {"project": _dict(project)}

            for table in tables:
                item[table] = [
                    _dict(row) for row in conn.execute(
                        f"SELECT * FROM {table} WHERE project_id=?", (pid,)
                    ).fetchall()
                ]
        ...
        with tempfile.NamedTemporaryFile(
            ...
            json.dump(result, temp, ensure_ascii=False, indent=2)
            temp.flush()
