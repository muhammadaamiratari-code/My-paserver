...
BACKUP_RECORD = os.path.join(PROJECT_ROOT, "backup_record.json")
MAX_BACKUPS = 20
...
def _safe_backup_path(file_path):
    base = os.path.realpath(BACKUP_FOLDER)
    path = os.path.realpath(file_path)
    if path != base and not path.startswith(base + os.sep):
        raise ValueError("Backup فولڈر سے باہر restore کی اجازت نہیں۔")
    return path
...
def create_backup(file_path, reason=""):
    try:
        ...
        backup_name = f"{os.path.basename(full_path)}.backup_{timestamp}"
        backup_path = os.path.join(BACKUP_FOLDER, backup_name)
        shutil.copy2(full_path, backup_path)
        record = _load_record()
        record["backups"].append({
            "timestamp": datetime.now().isoformat(),
            "original_file": os.path.relpath(full_path, PROJECT_ROOT),
...
def restore_backup(backup_path):
    try:
        backup_path = _safe_backup_path(backup_path)
        if not os.path.isfile(backup_path):
            return False, "Backup موجود نہیں۔"
        record = _load_record()
        info = None
        for item in record["backups"]:
            if os.path.realpath(item.get("backup_path", "")) == backup_path:
                info = item
                break
        if not info:
        ...
        if os.path.exists(original_path):
            ok, result = create_backup(original, reason="restore سے پہلے safety backup")
            if not ok:
...
def list_backups(file_path=None):
    try:
        ...
        results = [item for item in record["backups"] if os.path.normpath(str(item.get("original_file", ""))) == normalized]
        return True, results
    except Exception as e:
