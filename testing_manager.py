...
TEST_FOLDER = os.path.join(PROJECT_ROOT, "test_results")
TEST_RECORD = os.path.join(PROJECT_ROOT, "test_record.json")
...
def run_python_syntax(file_path):
    try:
        ...
        result = subprocess.run([sys.executable, "-m", "py_compile", full_path], capture_output=True, text=True, timeout=30)
        output = (result.stdout + result.stderr).strip()
        success = result.returncode == 0
        _log_test("python_syntax", file_path, success, output)
        return success, output if output else "Syntax درست ہے۔"
    except subprocess.TimeoutExpired:
        _log_test("python_syntax", file_path, False, "ٹائم آؤٹ")
...
def run_python_file(file_path, args=None):
    try:
        ...
        output = (result.stdout + result.stderr).strip()
        success = result.returncode == 0
        _log_test("python_run", file_path, success, output)
        return success, output if output else "کامیابی سے چل گیا۔"
    except subprocess.TimeoutExpired:
        _log_test("python_run", file_path, False, "ٹائم آؤٹ")
        return False, "Python program timeout۔"
    except Exception as e:
        return False, f"Run میں مسئلہ: {e}"


def check_server_health(url="http://127.0.0.1:5000/api/health"):
    try:
        ...
        with urllib.request.urlopen(url, timeout=10) as response:
            ...
            success = status == 200
            _log_test("server_health", url, success, data)
            return success, data
    except Exception as e:
        _log_test("server_health", url, False, str(e))
        return False, f"Server health میں مسئلہ: {e}"


def get_test_history(test_type=None, target=None):
    try:
        results = _load_record()["tests"]
        if test_type:
            results = [item for item in results if item.get("test_type") == test_type]
        if target:
            results = [item for item in results if item.get("target") == target]
        return True, results
    except Exception as e:
