...
SKIP_FOLDERS = {"dev_backups", "__pycache__", ".git", "node_modules", ".venv", "venv"}
os.makedirs(BACKUP_FOLDER, exist_ok=True)


def _safe_path(file_path):
    base = os.path.realpath(PROJECT_ROOT)
    path = os.path.realpath(file_path if os.path.isabs(file_path) else os.path.join(PROJECT_ROOT, file_path))
...
def _log_change(action, file_path, detail=""):
    data = _load_memory()
    data["changes"].append({"timestamp": datetime.now().isoformat(), "action": action, "file": str(file_path), "detail": str(detail)})
...
def find_errors(file_path):
    ...
    if long_lines:
        issues.append(f"120 characters سے لمبی لائنیں: {long_lines}")
    todo_lines = [i + 1 for i, line in enumerate(content.splitlines()) if "TODO" in line or "FIXME" in line]
    if todo_lines:
        issues.append(f"TODO/FIXME لائنیں: {todo_lines}")
    if not issues:
        return True, "کوئی واضح سطحی مسئلہ نہیں ملا۔"
    return True, issues


def edit_file(file_path, old_text, new_text, create_backup=True):
    try:
        full_path = _safe_path(file_path)
        if not os.path.isfile(full_path):
            return False, "فائل موجود نہیں۔"
        ...
        if occurrences > 1:
            return False, "پرانا متن ایک سے زیادہ جگہ موجود ہے؛ محفوظ edit کے لیے منفرد متن دیں۔"
        if create_backup:
            from backup_manager import create_backup as make_backup
            ok, result = make_backup(file_path, reason="قبل از edit")
            if not ok:
        ...
        _log_change("edit", file_path, "تبدیلی کامیابی سے محفوظ ہوئی۔")
        return True, "تبدیلی کامیاب رہی۔"
    except Exception as e:
        return False, f"تبدیلی میں مسئلہ: {e}"


def add_code(file_path, new_code, position="end", create_backup=True):
    try:
        full_path = _safe_path(file_path)
        if not os.path.isfile(full_path):
            return False, "فائل موجود نہیں۔"
        ...
        if create_backup:
            from backup_manager import create_backup as make_backup
            ok, result = make_backup(file_path, reason="قبل از add_code")
            if not ok:
                return False, f"Backup ناکام: {result}"
        with open(full_path, "r", encoding="utf-8") as f:
            content = f.read()
        ...
        with open(temp_path, "w", encoding="utf-8") as f:
            f.write(updated)
        os.replace(temp_path, full_path)
        _log_change("add", file_path, f"نیا code position={position}")
    ...
    try:
        ...
        for root, dirs, files in os.walk(PROJECT_ROOT):
            ...
            for name in files:
                ...
                files_list.append(os.path.relpath(path, PROJECT_ROOT))
